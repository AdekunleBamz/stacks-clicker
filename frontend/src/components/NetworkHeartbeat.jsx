...
const NetworkHeartbeat = memo(function NetworkHeartbeat() {
  ...
  return (
    <Tooltip content={heartbeatTitle}>
      <div className="heartbeat-container" tabIndex={0} aria-live="polite" aria-label={heartbeatTitle}>
        <div className={`heartbeat-pulse ${isConnected ? 'online' : 'offline'}`} aria-hidden="true"></div>
        <div className="heartbeat-info" aria-label="Network Vitality Metadata" aria-live={isConnected ? "polite" : "assertive"}>
