...
Clarinet.test({
  name: "edge: contract pause prevents interactions",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    chain.mineBlock([
      Tx.contractCall('clicker-v2p', 'pause', [], owner.address),
    ]);

    // Click should fail when paused
    let block = chain.mineBlock([
      Tx.contractCall('clicker-v2p', 'click', [], wallet1.address),
    ]);
    ...
    // Owner unpauses
    chain.mineBlock([
      Tx.contractCall('clicker-v2p', 'unpause', [], owner.address),
    ]);
...
Clarinet.test({
  name: "edge: non-owner cannot pause contract",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const wallet1 = accounts.get('wallet_1')!;

    let block = chain.mineBlock([
      Tx.contractCall('clicker-v2p', 'pause', [], wallet1.address),
    ]);
