# Rate-limit toast clarity

Use one consistent toast message for temporary rate limits across click, tip, and poll actions.
Consistent language helps users understand when retrying is safe.

- Re-test rapid duplicate notifications to keep message wording clear.

### Follow-up
- Reconfirm toast copy still matches active rate-limit behavior.
- Verify repeated rate-limit events do not stack multiple identical toasts.

Follow-up note: Keep toast copy concise enough to avoid truncation on small screens.
